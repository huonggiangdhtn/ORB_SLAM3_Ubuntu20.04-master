...
It reads the time stamps from the rgb.txt file and
...
def read_file_list(filename):
    ...
    list_ = [[v.strip() for v in line.split(" ") if v.strip() != ""] for line in lines if
            len(line) > 0 and line[0] != "#"]
    list_ = [(float(l[0]), l[1:]) for l in list_ if len(l) > 1]
    return dict(list_)
    # return list(list_)
...
#     # parse command line
#     parser = argparse.ArgumentParser(description='''This script takes two data files with timestamps and associates 
#     them''')
