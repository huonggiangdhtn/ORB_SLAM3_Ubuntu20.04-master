    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
 ...
 // Initialize pointcloud mapping
    //mpPointCloudMapping = make_shared<PointCloudMapping>( 0.04 );
    pointcloud_ = PointCloudMapping::Ptr(new PointCloudMapping(0.01));
    //Initialize the Tracking thread
    //(it will live in the main thread of execution, the one that called this constructor)
    cout << "Seq. Name: " << strSequence << endl;
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpAtlas, mpKeyFrameDatabase, strSettingsFile, mSensor, strSequence);
    mpTracker->setPointCloudMapping(pointcloud_);
...
{
    pointcloud_->save_pointcloud();
    mpLocalMapper->RequestFinish();
    mpLoopCloser->RequestFinish();
    pointcloud_->shutdown();
    if(mpViewer)
    {
